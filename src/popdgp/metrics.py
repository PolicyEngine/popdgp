"""Weighted joint, tail, and stress metrics for candidate populations.

A candidate synthetic population is a draw from an estimated distribution, so
these metrics score *distributional* fidelity, not point accuracy, and they do
so under survey weights -- the property common practice neglects.

Joint metrics (scored on a multivariate block; the population-view harness in
:mod:`popdgp.views` runs on these):
...
Tail metric (scored per imputed target by the view's tail block):

* :func:`weighted_wasserstein1` -- weighted Wasserstein-1 distance between two
  1-D samples, the single-number marginal-fit summary the tail block scales by
  the holdout's weighted sd.
...
* :func:`reweight_fragility` -- worst-case single-record share of a population
  ...
  a property of any weighted file, independent of any view.
